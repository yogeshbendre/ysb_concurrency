...
def CreateVDS(log,datacenter=None,vdsName=None,vdsVersion='5.0.0',vdsVendor='VMware'):
    ...
    if datacenter == None:
        ...
        log.info("THREAD - CreateVDS -  Datacenter name: " + datacenter.name)
    ...
    if v != None:
        log.warn("%s is existed in VC" %vdsName)
        return v
    ...
    try:
        ...
        log.info("THREAD - CreateVDS -  Creating a dvSwitch by name : " + vdsName + " version " + vdsVersion)
        vds.create(vdsName, datacenter, spec)
    except Exception as e:
        log.error(e)
        raise

    print("THREAD - CreateVDS -  Create VDS success")
    return vds.obj
...
def getPortName(datacenter,dvPortgroupSubstr,vdsName=None):
    if datacenter == None:
        print("specify datacenter object")
        return []
    if len(dvPortgroupSubstr) == 0:
        print("specify dvPortgroup name list")
        return []
    ...
    for child in children:
        ...
        if not (isinstance(child, vim.dvs.DistributedVirtualPortgroup)):
            continue
        if not dvPortgroupSubstr in child.name:
            continue
        if vdsName != None and vdsName != child.config.distributedVirtualSwitch.name:
            continue
        print child.name
    ...
    if len(pg) == 0:
        if vdsName == None:
            print("PortGroup with substring [" + str(dvPortgroupSubstr) + "] not found ")
        else:
            print("PortGroup with substring [%s] not found in VDS [%s]" % (str(dvPortgroupSubstr), vdsName))
        return []

    print("Found dvportgroup with  [" + str(dvPortgroupSubstr) + "] substring")
    return pg
