   ...
   def _Serialize(self, val, info, defNS):
      ...
      if isinstance(val, DataObject):
         ...
         if info.type is object:
            nsattr, qName = self._QName(Type(val), currDefNS)
            attr += '%s %stype="%s"' % (nsattr, self.xsiPrefix, qName)
         self.writer.write('<%s%s>%s</%s>' % (info.name, attr, val,
                                                               info.name))
      else:
         if info.type is object:
         ...
         self.writer.write(u'<%s%s>%s</%s>' % (info.name, attr, result,
                                               info.name))
...
class SSLTunnelConnection(object):
   ...
   def __call__(self, path, key_file=None, cert_file=None, context=None, **kwargs):
      ...
      if hasattr(retval, '_context'):
         ...
         if _CheckIPv4(urllib.splitport(self.proxyPath)[0]) or \
            _CheckIPv6(urllib.splitport(self.proxyPath)[0]) or \
            _CheckHostname(urllib.splitport(self.proxyPath)[0]):
         ...
         retval.sock = ssl.wrap_socket(tunnel.sock, keyfile=key_file,
                                       certfile=cert_file, **_sslArgs)
      return retval
...
class SessionOrientedStub(StubAdapterBase):
   ...
   def InvokeMethod(self, mo, info, args):
      ...
      while True:
         ...
         except (socket.error, six.moves.http_client.HTTPException, ExpatError) as ex:
