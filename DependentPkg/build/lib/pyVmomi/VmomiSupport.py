...
def CreateManagedType(vmodlName, wsdlName, parent, version, props, methods):
   with _lazyLock:
      ...
      if methods:
         for meth in methods:
            # We need to use method namespace to avoid duplicate wsdl method
            # issue when to find wsdl method from managed type namespace for
            # vSAN vmodls that can have different version namespaces for
            # managed methods and managed object type.
            methNs = GetWsdlNamespace(meth[2])
...
def CheckField(info, val):
   with _lazyLock:
      ...
      if val is None or (isinstance(val, list) and len(val) == 0):
         ...
         if isinstance(val, Array):
            ...
            if issubclass(valType.Item, info.type.Item):
               ...
               if issubclass(valType, GetVmodlType(info.expectedType)):
                  return
            elif issubclass(info.type.Item, Enum) and issubclass(valType.Item, six.string_types):
        ...
        or issubclass(info.type, six.string_types) and issubclass(valType, six.string_types) \
        or issubclass(info.type, six.binary_type) and issubclass(valType, six.binary_type):
         return
...
def GetServiceVersions(namespace):
   ...
   if version_info[0] >= 3:
      return sorted([v for (v, n) in six.iteritems(serviceNsMap) if n == namespace],
...
def GetWsdlMethod(ns, wsdlName):
   ...
   with _lazyLock:
      ...
      if isinstance(method, ManagedMethod):
         ...
         raise KeyError("%s %s" % (ns, wsdlName))
...
from .Version import Init as _VersionInit
_VersionInit()
...
if PY3:
   long = type("long", (int,), {})
URI = type("URI", (str,), {})
binary = type("binary", (six.binary_type,), {})
...
_wsdlNameMap[CreateArrayType(six.text_type)] = (XMLNS_VMODL_BASE, 'ArrayOfString')
